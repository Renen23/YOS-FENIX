...
async execute({ yosFenixBot, from, info, args, reply, reagir, isGroup, isAdm, isBotAdm, isDono, sender, groupMembers, botNumber, donoJid }) {
if (!isGroup) return reply('❌ Este comando só pode ser usado em grupos!')
if (!isAdm && !isDono) return reply('❌ Apenas administradores podem usar este comando!')
if (!isBotAdm) return reply('❌ A YOS FÊNIX precisa ser administradora para isso!')
...
await yosFenixBot.sendMessage(from, {text: `🌕 ${nomeTarget} está sendo removido do grupo...`, mentions: [targetRealJid]}, {quoted: info})
await new Promise(resolve => setTimeout(resolve, 1000))
await yosFenixBot.groupParticipantsUpdate(from, [targetRealJid], 'remove')
await reagir('✅')
} catch (error) {
await reagir('❌')
