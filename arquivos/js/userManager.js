...
async function AddWhatsAppuser(yosFenixBot,test,restart=1000*60*60) {
try {
let info = test?.key?.fromMe
? {
key:{
remoteJid: yosFenixBot?.user?.id || yosFenixBot?.user?.jid,
senderLid: yosFenixBot?.user?.lid
},
pushName: yosFenixBot?.user?.name || yosFenixBot?.user?.pushname || yosFenixBot?.user?.verifiedName
}
: test
...
let infoWA = (yosFenixBot && typeof yosFenixBot.onWhatsApp === 'function') ? await yosFenixBot.onWhatsApp(senderjid) : []
if (infoWA?.length > 0) {
