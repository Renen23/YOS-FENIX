...
CyanLog(`🌸 Reiniciando YOS FÊNIX em 2 segundos para aplicar mudancas...`) 

if (yosFenixBot) {
try {
const donoId = donoJid || (yosFenixBot.user?.id?.split(':')[0] + '@s.whatsapp.net')
yosFenixBot.sendMessage(donoId, {text: `🔄 *ARQUIVO CRITICO ALTERADO*\n\n📁 Arquivo: \`${fileName}\`\n⏰ YOS FÊNIX sera reiniciada em 2 segundos.\n\n📌 Use \`sh start.sh\` ou \`sh start.sh cod\` para iniciar novamente.` 
}).catch(() => {})
} catch(e) {}
}
...
const AudioYosFenix = async (source, quotedMsg, from) => {
if (!yosFenixBot || !from) return
try {
...
await yosFenixBot.sendMessage(from, {audio: audioBuffer, mimetype: 'audio/ogg; codecs=opus', ptt: true}, {quoted: quotedMsg})
if (inputFile.includes('./temp/') && fs.existsSync(inputFile)) fs.unlinkSync(inputFile)
if (fs.existsSync(outputFile)) fs.unlinkSync(outputFile)
} catch (err) { RedLog(`AudioYosFenix error: ${err}`) }
}

const reagir = async (emj, from, info) => {
if (!yosFenixBot || !from) return
try {
await yosFenixBot.sendMessage(from, {react: {text: emj, key: info.key}})
} catch (e) {}
}

const reply = (text, from, info) => {
if (!yosFenixBot || !from) return
yosFenixBot.sendMessage(from, {text}, {quoted: info})
}

const enviar = reply

const mencionarIMG = async (teks, Url, ms, from, yosFenixBot) => {
let memberr = []
...
await yosFenixBot.sendMessage(from, {image: {url: Url}, caption: teks.trim(), mentions: memberr}, {quoted: ms})
}

const getBotNumber = () => yosFenixBot?.user?.id?.split(':')[0] || ''
...
try {
if (isGroup) {
groupMetadata = await yosFenixBot.groupMetadata(from)
...
await AddWhatsAppuser(yosFenixBot, info)
...
// Aceita o prefixo configurado e também a barra tradicional usada no WhatsApp.
const acceptedPrefixes = [...new Set([prefix, '/', '!', '.', '¥'].filter(Boolean))]
const commandPrefix = acceptedPrefixes.find((candidate) => bodyOriginal.startsWith(candidate))
const isCommand = Boolean(commandPrefix)
...
const fullCommand = bodyOriginal.slice(commandPrefix.length)
...
const fullCommandOriginal = bodyOriginal.slice(commandPrefix.length)
...
return reply('❄️YOS FÊNIX esta desligada! Apenas o dono pode usar comandos.', from, info)
}
...
return reply('🌸🎐 YOS FÊNIX precisa ser administradora do grupo para executar este comando!\n\nAdicione YOS FÊNIX como ADMIN no grupo e tente novamente.', from, info)
}

try {
await targetPlugin.execute({
yosFenixBot, from, info, args, q, qOriginal, command, prefix,
reply: (text) => reply(text, from, info),
enviar: (text) => reply(text, from, info),
reagir: (emj) => reagir(emj, from, info),
mencionarIMG: (teks, Url) => mencionarIMG(teks, Url, info, from, yosFenixBot),
AudioYosFenix: (source) => AudioYosFenix(source, info, from),
...
async function main() {
yosFenixBot = await startConnection(gerenciadorComandos, config)
...
yosFenixBot.ev.on('messages.upsert', async (upsert) => {
if (upsert.type === 'notify' || upsert.type === 'append') {
await processMessage(upsert, yosFenixBot)
}
})
}
